...
export function calculateBmr(profile, weightKg) {
...
export function calculateDailyNutrition(profile, plan, meals, exercises, todayIso, daysRemaining) {
...
export function calculateWeightPrediction(records, plan, todayIso) {
    const sorted = [...records]
        .filter((record) => Number.isFinite(record.weight))
        .sort((a, b) => a.recordedOn.localeCompare(b.recordedOn))
        .slice(-14);
    if (sorted.length < 3) {
        return { available: false, reason: '3回以上の体重記録で予測を表示します。' };
    }
    ...
    if (spanDays < 2) {
        return { available: false, reason: '数日分の記録がたまると予測精度が上がります。' };
    }
